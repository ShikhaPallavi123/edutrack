...
db.serialize(() => {
  ...
  db.get('SELECT COUNT(*) as c FROM staff', (err, row) => {
    if (row && row.c === 0) {
      ...
      goals.forEach(g => {
        db.run(`INSERT INTO goals (staff_id,title,category,progress_pct,due_date) VALUES (?,?,?,?,?)`, g);
      });

      const obs = [
        [1,'2026-05-03','Grade 4 · Mathematics',4.2,4.5,4.0,4.2,3.8,4.5,'Clear objectives, strong use of manipulatives','Increase formative checks, extend wait time','Completed'],
        [2,'2026-04-22','Grade 7 · Earth Science',3.5,3.5,3.2,3.8,3.2,3.8,'Good content knowledge','Student engagement needs improvement','Completed'],
        [3,'2026-04-15','Grade 9 · English Lit',4.8,4.8,4.9,4.7,4.8,4.8,'Exceptional discussion facilitation','Minor pacing issues','Completed'],
      ];
      obs.forEach(o => {
        db.run(`INSERT INTO observations (staff_id,date,grade_subject,score,domain1,domain2,domain3,domain4,domain5,strengths,growth_areas,status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`, o);
      });
